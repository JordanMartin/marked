...
var block = {
  ...
  def: /^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +["(]([^\n]+)[")])? *(?:\n+|$)/,
  table: noop,
  paragraph: /^((?:[^\n]+\n?(?!hr|heading|lheading|blockquote|tag|def|plugin))+)\n*/,
  text: /^[^\n]+/,
  plugin: /^@(!?[A-Za-z0-9_-]+) *\( *((.+) *)?\).*?(\n(\s{2,}|\t{1,}).+\n(((\5).*\n)|\n)*)?/
};
...
function InlineLexer(links, options) {
  ...
  if (this.options.plugins) {
    this.rules.plugin = inline_plugin;
  }
}
...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (this.rules.plugin && (cap = this.rules.plugin.exec(src))) {
        src = src.substring(cap[0].length);
        out += this.renderer.plugin(cap[1], cap[2], false);
        continue;
    }
